...
class AddTransactionPage extends ConsumerStatefulWidget {
  final TransactionEntity? editingTransaction; // Untuk mode edit
...
class _AddTransactionPageState extends ConsumerState<AddTransactionPage> {
  ...
  String? _selectedType; // 'income' atau 'expense'
  BudgetEntity? _selectedBudget; // Hanya relevan untuk 'expense'
  ...
  void initState() {
    ...
    if (widget.editingTransaction != null) {
      ...
      if (widget.editingTransaction!.budgetId != null && widget.editingTransaction!.budgetId!.isNotEmpty) {
      ...
      _selectedDate = DateTime.now(); // Default tanggal hari ini untuk transaksi baru
      _selectedType = 'expense'; // Default ke expense (bisa diubah ke 'income' jika Anda lebih suka)
    }
  }
  ...
  void _saveTransaction() async {
    ...
    // Dapatkan ID pengguna saat ini dari Supabase Auth
    // Jika tidak ada pengguna login (anonim), userId akan null
    final currentUserId = Supabase.instance.client.auth.currentUser?.id;

    final transactionToSave = TransactionEntity(
      id: widget.editingTransaction?.id ?? '',
      // userId akan menjadi null jika tidak ada sesi aktif,
      // sesuai dengan kolom user_id di Supabase yang sudah nullable.
        ...
        child: SingleChildScrollView(
          child: Form(
            ...
            child: Column(
              ...
              children: [
                Text(
                  widget.editingTransaction == null ? 'Catat Pemasukan atau Pengeluaran' : 'Perbarui Transaksi',
                    ...
                    if (cleanValue.isNotEmpty) {
                      try {
                        ...
                        _amountController.value = TextEditingValue(
                          text: NumberFormat.decimalPattern('id_ID').format(parsed),
                          selection: TextSelection.collapsed(offset: NumberFormat.decimalPattern('id_ID').format(parsed).length),
                        );
                      } catch (_) {
                        // Biarkan input tidak berubah jika parsing gagal
                      }
                    }
                  },
                  validator: (value) {
